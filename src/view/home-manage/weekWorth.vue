...
<template>
  <div class="add">
      <Form ref="titleform" :model="titleform" :rules="titleFormValidate" inline :label-width="50">
    ...
    <Modal v-model="editModal" width="600" class="hideFootModal" :title="modalTitle">
      <Form ref="formValidate" :model="detailForm" :rules="ruleValidate" :label-width="80">
        ...
        <FormItem label="跳转类型" prop="plateType" on-change="changeType">
...
export default {
  ...
  methods: {
    ...
    async getCheckTitleMsg () {
      let res = await doCheckTitleMsg({ showMode: 4, FLAG: 1 })
      if (res.data.code === 0) {
